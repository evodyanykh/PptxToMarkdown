...
class PPTXParser:
    ...
    def _get_slide_paths_safe(self):
        ...
        """
        try:
            # Попробуем через presentation.xml + rels
            ...
            for sld in pres_root.findall(".//p:sldId", ns_pres):
                ...
                if slide_target:
                    # иногда Target = "../slides/slideX.xml"
            ...
            # fallback: просто возьмём все slide*.xml
    ...
    def _parse_slide(self, xml_bytes: bytes, index: int):
        ...
        for z_index, shape in enumerate(root.findall(".//p:sp", ns)):
            text = self._extract_text(shape, ns)
            if not text:
                continue
            ...
            blocks.append(
                TextBlock(
                    slide_index=index,   # ✅ обязательно передаем индекс слайда
                    text=text,
                    x=x,
                    y=y,
                    width=w,
                    height=h,
                    z=z_index  # порядок в XML = визуальный слой
                )
            )
            # 👉 если это заголовок слайда — сохраняем отдельно
            #if self._is_slide_title(shape, ns):
                # берём первый непустой параграф как заголовок
    ...
    def _extract_text(self, shape, ns) -> str:
        """
